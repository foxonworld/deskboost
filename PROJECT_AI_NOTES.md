...
- Plant Profile FE-first care profile decision: `FE/pages/PlantProfile.jsx` must not use fake `plant.nextWatering`; it derives next/last watering and upcoming care from real `GET /api/reminders`, filters recent AI dialogs from `GET /api/ai/dialogs`, and falls back from `GET /api/my-plants/{id}` to the real `GET /api/my-plants` list if the detail endpoint fails.
