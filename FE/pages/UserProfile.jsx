      ...
      <div className="p-8 max-w-6xl mx-auto space-y-10 pb-20">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          ...
          <div className="flex gap-3">
            <button className="px-6 py-3 rounded-2xl border border-slate-200 dark:border-slate-800 text-slate-500 font-black text-xs uppercase tracking-widest hover:bg-slate-50 transition-all">{t('userProfile.cancel')}</button>
            <button className="px-8 py-3 rounded-2xl bg-[#4CAF50] text-white font-black text-xs uppercase tracking-widest shadow-xl shadow-[#4CAF50]/20 hover:scale-105 active:scale-95 transition-all">{t('userProfile.save')}</button>
          </div>
        </div>
            ...
            <div className="bg-white dark:bg-slate-900 p-10 rounded-[40px] border border-slate-50 dark:border-slate-800 shadow-sm text-center relative overflow-hidden group">
              ...
              <div className="relative inline-block mb-6">
                <div className="size-36 rounded-full border-4 border-[#4CAF50] p-1.5 shadow-2xl">
                  <img alt={t('userProfile.imageAlt')} className="w-full h-full object-cover rounded-full" src="https://lh3.googleusercontent.com/aida-public/AB6AXuBwtOPjZ2-FOeAcMDcmIhMcUzzaLNQrs8BptWKABgS5XBqaKTZ9fURtN5btwU4zbVYIL9zXEttn-r1pqc96x16chz_YvxcevznFXhcFmnNTwdqsmygM0C4eP48o0LjWtDBYoG_kCbm2l3ErAZzMZbm94A5gpamq_7gX44kFE0ZbPWazZhOAPEusmYenNzdAQxOAzdd2HIadO3xpslF6ZWnbNMrD0llQhqRHltEjAaHT7O5Wyrtxa0DEs_JNU-_CcX717MUC9dFat04"/>
                </div>
          ...
          <div className="lg:col-span-2 space-y-8">
            <div className="bg-white dark:bg-slate-900 p-10 rounded-[40px] border border-slate-50 dark:border-slate-800 shadow-sm space-y-10">
              <div className="space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="md:col-span-2 space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('userProfile.motto')}</label>
                    <textarea className="w-full rounded-2xl border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 p-6 text-sm font-bold focus:ring-4 focus:ring-[#4CAF50]/5 outline-none resize-none" placeholder={t('userProfile.mottoPlaceholder')} rows={4}></textarea>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-8 pt-10 border-t border-slate-50 dark:border-slate-800">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('userProfile.currentPassword')}</label>
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('userProfile.newPassword')}</label>
                    <input className="w-full h-14 rounded-2xl border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 px-6 text-sm font-bold" placeholder={t('userProfile.newPasswordPlaceholder')} type="password"/>
                  </div>
                </div>
              </div>
            </div>
