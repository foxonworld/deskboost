                  ...
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      ...
                      <div className="mt-2 flex flex-wrap gap-2">
                        <span className="rounded-full bg-slate-100 px-3 py-1 text-[10px] font-black uppercase tracking-wider text-slate-500 dark:bg-slate-800 dark:text-slate-300">Claim: {plant.ownershipStatus || plant.claimCodeStatus || 'Unknown'}</span>
                        {(plant.ownershipCode || plant.claimCode) && (
                          <span className="rounded-full bg-slate-100 px-3 py-1 text-[10px] font-black uppercase tracking-wider text-slate-500 dark:bg-slate-800 dark:text-slate-300">Code: {plant.ownershipCode || plant.claimCode}</span>
                        )}
                      </div>
                    </div>
              ...
              <div className="mt-4 space-y-4">
                ...
                <section className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-950/50">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-sm font-black text-slate-900 dark:text-white">Claim code</p>
                      <p className="mt-1 text-xs font-bold leading-5 text-slate-500 dark:text-slate-400">Code sent to the buyer and used to add this plant.</p>
                    </div>
                  </div>
                  <div className="mt-4 grid gap-2 sm:grid-cols-2">
                    <div className="rounded-xl bg-white p-3 dark:bg-slate-900">
                      <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Code</p>
                      <p className="mt-1 text-sm font-bold text-slate-500">{selectedPlant.ownershipCode || selectedPlant.claimCode || 'Not linked'}</p>
                    </div>
                    <div className="rounded-xl bg-white p-3 dark:bg-slate-900">
                      <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Claim status</p>
                      <p className="mt-1 text-sm font-bold text-slate-500">{selectedPlant.ownershipStatus || selectedPlant.claimCodeStatus || 'Unknown'}</p>
                    </div>
                  </div>
                </section>
              </div>
