...
const AdminUsers = () => {
  ...
  const openDetail = async (userId) => {
    ...
    try {
      ...
      setDetailError(err?.message || 'Could not load user detail.');
    } finally {
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      ...
      setStatusError(err?.message || 'Could not update user status.');
    } finally {
    ...
    <AdminLayout>
      <section className="rounded-[32px] border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 sm:p-8 shadow-sm">
        ...
        <p className="mt-3 max-w-2xl text-sm font-medium leading-6 text-slate-500 dark:text-slate-400">
          Basic account list, detail, and status management from real admin endpoints.
        </p>
        <p className="mt-4 rounded-2xl bg-amber-50 px-4 py-3 text-sm font-bold text-amber-700 dark:bg-amber-950/30 dark:text-amber-300">
          Create, edit profile fields, and delete user need backend endpoints. No mock admin user CRUD is saved here.
        </p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4">
          <div className="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-[28px] border border-slate-100 bg-white p-6 shadow-2xl dark:border-slate-800 dark:bg-slate-900 sm:p-8">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-xs font-black uppercase tracking-[0.3em] text-[#4CAF50]">User detail</p>
                <h2 className="mt-3 text-2xl font-black text-slate-900 dark:text-white">Account controls</h2>
              </div>
              <button
                type="button"
                onClick={() => {
            ...
            {detailLoading ? (
              <p className="mt-5 text-sm font-bold text-slate-400">Loading user detail...</p>
            ) : detailError ? (
              ...
              <div className="mt-5 grid gap-5 lg:grid-cols-[1.2fr_0.8fr]">
                <div className="rounded-2xl border border-slate-100 p-5 dark:border-slate-800">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-lg font-black text-slate-900 dark:text-white">{selectedUser.name || 'Unnamed user'}</p>
                      <p className="text-sm font-semibold text-slate-500">{selectedUser.email}</p>
                    </div>
                  </div>
                  <dl className="mt-5 grid gap-3 text-sm sm:grid-cols-2">
                    <div>
                      <dt className="font-black text-slate-400">Role</dt>
                      <dd className="mt-1 font-bold text-slate-700 dark:text-slate-200">{selectedUser.role || 'N/A'}</dd>
                    </div>
                    <div>
                      <dt className="font-black text-slate-400">Status</dt>
                      <dd className="mt-1 font-bold text-slate-700 dark:text-slate-200">{selectedUser.status || 'N/A'}</dd>
                    </div>
                    <div>
                      <dt className="font-black text-slate-400">Phone</dt>
                      <dd className="mt-1 font-bold text-slate-700 dark:text-slate-200">{selectedUser.phone || 'N/A'}</dd>
                    </div>
                    <div>
                      <dt className="font-black text-slate-400">Created</dt>
                      <dd className="mt-1 font-bold text-slate-700 dark:text-slate-200">{selectedUser.createdAt || 'N/A'}</dd>
                    </div>
                  </dl>
                </div>

                <div className="rounded-2xl border border-slate-100 p-5 dark:border-slate-800">
                  <label className="text-sm font-black text-slate-700 dark:text-slate-200" htmlFor="admin-user-status">Status</label>
                  <select
                  ...
                  <p className="mt-4 rounded-2xl bg-slate-50 px-4 py-3 text-xs font-bold leading-5 text-slate-500 dark:bg-slate-950 dark:text-slate-400">
                    Edit user fields, create account, and delete account are backend-required actions and are intentionally not mocked.
                  </p>
                </div>
              </div>
            )}
