      ...
      <div className="md:hidden">
        ...
        {isMobileOpen && (
          <div className="fixed inset-0 z-30 bg-slate-950/40 px-4 pb-20 pt-24 backdrop-blur-sm" onClick={closeMobile}>
            <nav className="max-h-full overflow-y-auto rounded-3xl border border-[#E4EEE6] bg-surface-light p-3 shadow-xl dark:border-[#2A4532] dark:bg-surface-dark" aria-label="User mobile navigation" onClick={(event) => event.stopPropagation()}>
              <div className="mb-2 flex items-center justify-between px-2 py-1">
                <p className="text-xs font-bold tracking-wide text-primary">Điều hướng</p>
        ...
        <div className="space-y-4 border-t border-[#E4EEE6] p-4 dark:border-[#2A4532]">
          <div className="flex items-center gap-3 p-2 min-w-0">
            ...
            <div className="flex flex-col min-w-0">
              <span className="max-w-[150px] truncate text-sm font-bold dark:text-white">{user?.name || 'Tài khoản'}</span>
              <span className="max-w-[150px] truncate text-[11px] font-semibold text-slate-400">{user?.email || 'DeskBoost MVP'}</span>
            </div>
          </div>
          <button
            disabled={isLoading}
          ...
          >
            ...
            <span>{isLoading ? 'Đang xuất...' : 'Đăng xuất'}</span>
          </button>
        </div>
      </aside>
    </>
